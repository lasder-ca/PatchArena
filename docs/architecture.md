...
The definition is limited to 100 unique tasks and the complete Cartesian plan to 1,000 agent invocations. The runner exposes plans through read-only accessors and revalidates their task shape, fingerprint, identities, agent order, repeat count, and checked invocation count before creating the initial checkpoint; the core checkpoint model independently enforces the same invocation ceiling. Execution is sequential so checkpoint order is deterministic and existing process/resource controls apply to one cell at a time. Repository `HEAD` and the task benchmark identity are checked both before and after every group, so drift during the final cell also aborts the durable checkpoint. A completed cell points to exactly one immutable group UUID; an orchestration error stores a bounded diagnostic; a pending cell has neither. Resume reconstructs preflight and proceeds only when the suite fingerprint, committed revision, per-task benchmark identities, ordered agent IDs, repeat count, and instruction condition still match. It never reruns a terminal cell.
...
Report generation consumes persisted records only; it does not rerun agents or verification commands. Suite-run child directories must be ordinary contained directories when read or resumed, and each checkpoint replacement rechecks both that directory and the existing regular checkpoint file. A suite report accepts only completed groups referenced by its checkpoint and revalidates task, agent, benchmark identity, instruction policy, requested/observed counts, and aggregate values against per-run evidence. Extra, duplicate, missing, or incompatible groups are errors. JSON is intended for automation, Markdown for review, and HTML for a portable local report. HTML output embeds its styles, requires no external assets, and must escape all repository-, task-, command-, and agent-controlled text.
