...
A suite multiplies privileged agent work as `tasks × agents × repeat`. PatchArena requires an explicit non-duplicate agent list, limits definitions to 100 tasks, caps a plan at 1,000 agent invocations, and offers `--dry-run` to validate and print that count before any run/group/suite-run record is created. Plans are externally read-only and the runner recomputes their shape and checked count before execution; the persisted core model separately rejects a larger matrix. These controls reduce accidental multiplication; they do not estimate provider currency cost, tokens, rate limits, network use, or cumulative host resources. An operator must choose an affordable plan and apply an external overall budget/deadline. A malicious task can still make each allowed invocation expensive.
...
`suite.json` is an unsigned local checkpoint, not an attestation. It is schema-validated, tied to its UUID path, and atomically replaced after every terminal cell. Suite-run child-directory links are rejected before checkpoint reads and again before replacements. A completed cell must reference one group UUID; pending and error cells cannot smuggle group evidence. Resume re-runs preflight and refuses identity drift in the committed `HEAD`, suite fingerprint, task/effective-policy fingerprints, ordered agents, repeat count, or instruction condition. Execution checks `HEAD` and task identity before and after each group, aborting even if drift occurs during the last cell, and resume invokes only pending cells. This prevents accidental continuation under changed inputs but does not stop a local attacker with write access from replacing a coherent set of checkpoint, group, and run records or winning a check-to-use race.
